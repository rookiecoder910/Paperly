    ...
    <section className="relative flex flex-col items-center overflow-hidden px-4 pt-24 pb-16 sm:pt-28 sm:pb-24 lg:pt-32 lg:pb-32">
      ...
      >
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[240px_1fr_260px] lg:gap-5">
          ...
          >
            <h3 className="mb-4 text-sm font-semibold text-white/90">Handwritten Notes</h3>

            {/* Folders */}
            <div className="mb-5">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs font-medium text-white/50">Folders</span>
                <svg className="h-3.5 w-3.5 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </div>
              <div className="space-y-1.5">
                <div className="rounded-lg bg-indigo-500/20 border border-indigo-500/30 px-3 py-2 text-xs text-indigo-300 font-medium">Personal</div>
                <div className="rounded-lg px-3 py-2 text-xs text-white/50 hover:bg-white/5 transition-colors">Work</div>
              </div>
            </div>
            ...
            <div className="mb-5">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-xs font-medium text-white/50">Recent Notes</span>
                <svg className="h-3.5 w-3.5 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </div>
              <div className="space-y-1">
                {["Handwritten Notes - ...", "Handwritten Notes - 1...", "Handwritten pages - 1...", "Handwritten pages - 1...", "Handwritten pages - 1..."].map((note, i) => (
                  <div key={i} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-[11px] text-white/40 hover:bg-white/5 transition-colors">
            ...
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-white/50">Tags</span>
                <svg className="h-3.5 w-3.5 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="flex items-center gap-1 border-b border-white/[0.06] px-3 py-2 sm:px-4">
              ...
              <div className="flex items-center gap-0.5 rounded-md border border-white/10 bg-white/[0.04] px-2 py-1">
                <span className="text-[10px] text-white/50">Edit</span>
                <svg className="h-3 w-3 text-white/30" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
              </div>
          ...
          >
            <h3 className="mb-4 text-sm font-semibold text-white/90">OCR & Processing</h3>

            {/* Upload area */}
            <div className="mb-5 rounded-lg border border-dashed border-white/15 bg-white/[0.02] p-5 text-center">
              ...
              <p className="text-xs font-medium text-white/70">Upload Document</p>
              <p className="mt-1 text-[10px] text-white/35">Drop your image or paste text, or concept there.</p>
            </div>

            {/* Processing progress */}
            <div className="mb-5">
              <div className="mb-2 flex items-center justify-between">
                <span className="text-[11px] text-white/50">Processing...</span>
                <span className="text-[11px] text-white/50">43%</span>
              </div>
              <div className="h-1.5 w-full rounded-full bg-white/[0.06] overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: "0%" }}
                  animate={{ width: "43%" }}
                  transition={{ duration: shouldReduce ? 0.01 : 1.5, delay: shouldReduce ? 0 : 0.8, ease: "easeOut" }}
                />
              </div>
            </div>
...
function ToolbarBtn({ children, bold, italic }: { children: React.ReactNode; bold?: boolean; italic?: boolean }) {
  return (
    <button className={`flex h-6 w-6 items-center justify-center rounded text-[10px] text-white/50 hover:bg-white/10 hover:text-white/80 transition-colors ${bold ? "font-bold" : ""} ${italic ? "italic" : ""}`}>
      {children}
    </button>
  );
}
