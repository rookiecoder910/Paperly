...
function FeatureCard({ title, description, bullets }: FeatureCardProps) {
  return (
    <div className="dashboard-panel rounded-xl p-6 sm:p-8 transition-transform hover:scale-[1.02]">
      <h3 className="mb-3 text-lg font-bold tracking-tight text-foreground/90 sm:text-xl">
        {title}
      </h3>
      ...
      {bullets && bullets.length > 0 && (
        <ul className="space-y-2">
          {bullets.map((b) => (
            <li key={b} className="flex items-start gap-2 text-xs text-muted-foreground/70 sm:text-sm">
